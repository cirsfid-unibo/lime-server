...
let $documentName := functx:substring-after-last($requestedFile, '/')
let $metadataFile := replace('.metadata.%name%', '%name%', $documentName)
let $metadataPath := functx:substring-before-last($requestedFile, '/')
