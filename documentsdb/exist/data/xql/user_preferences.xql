...
declare variable $existUsersPreferencesCollection external;
declare variable $userName external;
declare variable $userFullName external;
declare variable $lastOpened external;
declare variable $views external;
declare variable $defaultLanguage external;
declare variable $defaultLocale external;

(: Login credentials (as admin) :)
declare variable $existAdminUser external;
declare variable $existAdminPassword external;

(:  Get the requested action :)
declare variable $requestedAction external;
...
as node()* {
    ...
    let $relativePath := concat(replace($existUsersPreferencesCollection, '/db', ''), '/', $resourceName)
...
as xs:string {
    ...
    let $realFullName :=
        if ($resourceExists and string-length($userFullName) eq 0) then
    ...
    return
        if ($resourceExists and not($overwrite)) then
            ...
            xmldb:store($existUsersPreferencesCollection, $resourceName, $newUserXml)
};
...
as element()* {
    ...
    let $realCollection := collection($existUsersPreferencesCollection)
...
let $login := xmldb:login('/db', $existAdminUsdeclare variable $existAdminPassword)external;
